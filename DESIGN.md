...
- **M3/M4 (done):** walk-forward validation harness (`walkforward.py`) — rolling/anchored train+test splitter with embargo, in-sample `SignalParams` tuning, pooled out-of-sample evaluation, cost-sensitivity sweep, and a consolidated GO/NO-GO report combining OOS expectancy, in-sample→OOS decay, cost survival, and Monte-Carlo P(pass)/P(breach) under the confirmed rules. Runner: `examples/walkforward.py`. (Still needs real perp data to produce a meaningful verdict.)
